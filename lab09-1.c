#pragma config(Motor,  motorA,          J1,            tmotorNXT, PIDControl, encoder)
#pragma config(Motor,  motorC,          J2,            tmotorNXT, PIDControl, encoder)
//*!!Code automatically generated by 'ROBOTC' configuration wizard               !!*//

#define LENGTH 41

#define J1_INIT 0
#define J2_INIT 180

#define J1_SCALE 1
#define J2_SCALE 1

int path[LENGTH][2] = {{13,146},
{18,132},
{22,122},
{26,112},
{28,104},
{31,96},
{33,89},
{35,83},
{37,76},
{38,70},
{40,64},
{41,58},
{41,52},
{41,47},
{41,41},
{40,35},
{38,29},
{34,23},
{36,16},
{39,9},
{36,0},
{53,0},
{85,9},
{87,4},
{90,0},
{92,-4},
{94,-9},
{96,-14},
{98,-18},
{100,-23},
{95,-32},
{93,-40},
{93,-48},
{93,-57},
{94,-65},
{96,-73},
{99,-82},
{102,-92},
{105,-102},
{109,-114},
{114,-128},
}



void goToTheta(int t1, int t2){
	while(nMotorEncoder(J1)!=t1-J1_INIT||nMotorEncoder(J2)!=t2-J2_INIT){
		
		nxtDisplayClearTextLine(5);
		nxtDisplayClearTextLine(6);
		nxtDisplayString(5,"%d",nMotorEncoder(J1));
		nxtDisplayString(6,"%d",nMotorEncoder(J2));
		
		if(nMotorEncoder(J1) < t1-J1_INIT)
				motor[J1] = 10;
		else if(nMotorEncoder(J1) > t1-J1_INIT)
				motor[J1] = -10;
		else
				motor[J1] = 0;
		
		if(nMotorEncoder(J2) < t2-J2_INIT)
				motor[J2] = 10;
		else if(nMotorEncoder(J2) > t2-J2_INIT)
				motor[J2] = -10;
		else
				motor[J2] = 0;
	}
	motor[J1] = 0;
	motor[J2] = 0;
}

task main()
{
	nMotorEncoder(J1) = 0;
	nMotorEncoder(J2) = 0;
	for(int i = 0; i< LENGTH; i++){
		goToTheta((int)path[i][0], (int)path[i][1]);
	}
	while(1){
		nxtDisplayClearTextLine(5);
		nxtDisplayClearTextLine(6);
		nxtDisplayString(5,"%d",nMotorEncoder(J1));
		nxtDisplayString(6,"%d",nMotorEncoder(J2));
		
	};
}
